...
class OsdWindow extends Clutter.Actor {
    _init(monitorIndex) {
        ...
        // Заменяем вертикальный контейнер на горизонтальный для уровня и метки
        this._levelContainer = new St.BoxLayout({
            vertical: false, // Горизонтальное расположение
            y_align: Clutter.ActorAlign.CENTER,
            style_class: 'osd-level-container',
        });
        this._hbox.add_child(this._levelContainer);

        this._level = new BarLevel.BarLevel({
            style_class: 'level',
            value: 0,
        });
        this._levelContainer.add_child(this._level);

        this._label = new St.Label({
            style_class: 'osd-percentage-label',
        });
        this._levelContainer.add_child(this._label);
    ...
    setLevel(value) {
        ...
        if (this._level.visible) {
            value = value / 100; // Нормализуем значение к диапазону [0, 1]
            if (this.visible)
            ...
            // Обновляем текст метки с процентом громкости
            const percentage = Math.round(value * 100);
            const labelFormat = this._osdSettings?.get_string("volume-label-format") || "{percentage}%";
            this.setLabel(labelFormat.replace("{percentage}", percentage));
        }
    }
